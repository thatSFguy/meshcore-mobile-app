 ...
 *
 * A mesh this table does not cover is served by a scanned settings QR
 * ([ShareUri] `radio/set`), not by adding a row here. A hardcoded entry
 * goes stale silently and can only be corrected by shipping a build —
 * which is the wrong shape for something a local community changes.
 *
